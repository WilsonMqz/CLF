...
def read_data_smoking_TF_MSL(i_train_iter, n):
    random.seed(i_train_iter)
    P0 = 0
    P1 = 1
    P2 = 2
    P3 = 3
    train_datasets = 'D:/code/data/smoking/dataset/training_data'
    test_datasets = 'D:/code/data/smoking/dataset/testing_data'
    ...
    for i in range(train_label.size()[0]):
        if train_label[i] == P0 or train_label[i] == P1 or train_label[i] == P2 or train_label[i] == P3:
            index.append(i)
    ...
    num_t_label = torch.eq(new_train_TF_label_index, 3).sum()
    ...
    data_prior_hide = (num_0_label+ num_1_label+num_2_label - num_H0_label-num_H1_label-num_H2_label)/num_t_label.float()
    ...
    for i in range(test_data.size()[0]):
        if test_label[i] == P0 or test_label[i] == P1 or test_label[i] == P2 or test_label[i] == P3:
            index.append(i)
    index = torch.tensor(index)
